...
The default agent repeatedly executes every returned tool call, appends `role: tool` results, and calls the model again until a final answer or the six-step cap. It then verifies citations and relevance, retries once with feedback, and otherwise returns an explicit insufficient-evidence answer. Session-scoped recent turns support follow-ups, and every request automatically writes a JSON trajectory. The exact original first-tool-only router remains available with `?mode=baseline`.
