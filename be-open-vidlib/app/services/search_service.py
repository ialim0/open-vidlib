...
def _overlapping_sentences(
    transcript_segments: Optional[List[Dict[str, Any]]],
    chunk_start: float,
    chunk_end: float,
) -> List[Dict[str, Any]]:
    """Return transcript sentences that overlap a chunk's time range."""
    ...
    for seg in transcript_segments:
        ...
        if not seg_text:
            continue
        # Sentence overlaps if it starts before chunk ends AND ends after chunk starts
...
def _result(
    ...
    sentences = _overlapping_sentences(
        transcript_segments, segment.start_time, segment.end_time
    )
    if sentences:
...
def search_video(video_id: str, query: str, db: Session, top_k: int = 5) -> List[Dict[str, Any]]:
    ...
    # Look up sentence-level timestamps from the transcript (single query, cached for all results)
    transcript = db.query(Transcript).filter(Transcript.video_id == video_id).first()
    transcript_segments = transcript.segments if transcript and transcript.segments else None

    return [_result(segment, fused[segment.id] / maximum, transcript_segments) for segment in selected]
